...
function Section({
  ...
  return (
    <div className={`${pageBreak ? 'page-break' : ''} rounded-2xl border border-slate-200 bg-slate-50 p-4 space-y-3`}>
...
export function InvestorReportTemplate({ data, meta }: { data: ReportData; meta?: { snapshotId?: string | null; generatedAt?: Date } }) {
  ...
  return (
    <div className="bg-white text-slate-900 font-sans w-full max-w-none mx-auto p-10 space-y-8">
      {/* Page 1 */}
      <div className="space-y-4">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-2">
            ...
            <div className="text-[11px] text-slate-600">{movementBadge}</div>
          </div>
      ...
      {/* Appendix */}
      <div className="page-break rounded-2xl border border-slate-200 bg-slate-50 p-4 space-y-2">
        <div className="text-sm font-semibold text-slate-900">Assumptions & methodology</div>
        <ul className="text-xs text-slate-700 list-disc pl-4 space-y-1">
          {scenarioNotes.map((a, idx) => (
            <li key={idx}>{a}</li>
        ...
        <div className="text-sm font-semibold pt-3 text-slate-900">Data quality</div>
        <ul className="text-xs text-slate-700 list-disc pl-4 space-y-1">
          <li>Mapping completeness: {(dataQuality.mappingCompleteness * 100).toFixed(0)}%</li>
          <li>Missing key accounts: {dataQuality.missingKeyAccounts.length}</li>
          {missingMapping.length > 0 && <li>Examples to map: {missingMapping.join(', ')}</li>}
